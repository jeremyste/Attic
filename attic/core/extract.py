...
def extract_fat(image: str, dest_dir: str, *, timeout: float | None = None) -> ExtractResult:
...
def extract_mount(
    image: str, dest_dir: str, fstype: str, *, timeout: float | None = None
) -> ExtractResult:
    ...
    mount_argv = su.with_pkexec(["mount", "-o", "ro,loop", "-t", fstype, image, mnt])
...
def extract(
    ...
    if fstype.lower() in FAT_FSTYPES:
        return extract_fat(image, dest_dir, timeout=timeout)
    return extract_mount(image, dest_dir, fstype, timeout=timeout)
